...
public class LecDAOImpl implements ILecDAO {
	...
	public void updateCnt(int countNo) {
		...
		SqlSession sqlSession = MyBatisUtil.getInstance();
		try {
			sqlSession.update("lecture.updateCnt", countNo);
			sqlSession.commit();
		} catch (PersistenceException e) {
			e.printStackTrace();
			sqlSession.rollback();
	...
	public void lecScore(int lecNo) {
		...
		SqlSession sqlSession = MyBatisUtil.getInstance();
		try {
			sqlSession.update("lecture.lecScore", lecNo);
			sqlSession.commit();
		} catch (PersistenceException e) {
			e.printStackTrace();
			sqlSession.rollback();
	...
	@Override
	public void updateScore(String lecNo) {
//		int cnt = 0;
//		SqlSession sqlSession = MyBatisUtil.getInstance();
//		try {
//			cnt = sqlSession.update("lecture.updateScore", lecNo);
//			if(cnt > 0) sqlSession.commit();
//		} catch (PersistenceException e) {
//			e.printStackTrace();
//		}finally {
