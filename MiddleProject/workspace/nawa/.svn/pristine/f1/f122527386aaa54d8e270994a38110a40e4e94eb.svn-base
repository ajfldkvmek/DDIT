...
//		System.out.println("post왔어양!!!!!!");
		...
		add1 = add1+" "+add2;
		if( (req.getParameter("mem_add2") != null)){
		...
		if(memService.insertMember(mv) != 0) {
			...
			out.println("<script>alert('회원가입성공 로그인페이지로 갑니다!'); location.href = './member/signin.jsp'</script>");
			out.flush();
		} else {
			PrintWriter out = resp.getWriter();
			out.println("<script>alert('회원가입실패 회원가입페이지로 갑니다!'); location.href = './member/signUp.jsp'</script>");
			out.flush();
		}
	}
}
