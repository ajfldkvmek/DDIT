			...
			<thead class="table-dark">
				<tr>
					...
					<form action="./catelist.do" method="post" enctype="multipart/form-data" name="postCate"
						style="height:28px; padding:0px; margin:0px;">
								<select name="postCate" id="postCate" class="form-select form-select" 
									style="margin: top; transform: translateY(-13%); color:black;"
...
<%
    System.out.println("[catelist.jsp] currentPage: " + currentPage);
    System.out.println("[catelist.jsp] totalPages: " + totalPages);
%>
	...
	%>
			...
			<li class="page-item">
			<a href="./catelist.do?postCate=<%=pv.getCateValue() %>&catePage=<%= currentPage - 1 %>" class="page-link">이전</a>
			</li>
	<%
		}else{
	%>
	...
	%>
		<li class="page-item">
		<a href="./catelist.do?postCate=<%=pv.getCateValue() %>&catePage=1" class="page-link">1</a>
		</li>
	<%
	}
	if (startPage > 2) {
	...
	%>
			...
			<li class="page-item">
			<a href="./catelist.do?postCate=<%=pv.getCateValue() %>&catePage=<%= i %>" class="page-link"><%= i %></a>
			</li>
	<%
		}
	}
	...
	%>
		<li class="page-item">
			<a href="./catelist.do?postCate=<%=pv.getPostCate() %>&cateNo=<%=pv.getCateNo() %>&catePage=<%= totalPages %>" class="page-link"> <%= totalPages %></a>
		</li>
	<%
	}
	%>
...
<!-- '다음' 버튼 -->
		...
		%>
			<li class="page-item">
				<a href="./catelist.do?postCate=<%=pv.getCateValue() %>&catePage=<%= currentPage + 1 %>" class="page-link">다음</a>
			</li>
		<%
		}else{
