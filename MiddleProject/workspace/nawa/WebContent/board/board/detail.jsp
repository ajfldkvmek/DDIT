...
<%-- 			<td class="detailCate" width=15% colspan="2"><%=pv.getPostDate() %> --%>
			...
			<td colspan="5"><%=pv.getPostContent()%>
				...
				%>
					<div>
						...
						<img src="<%=request.getContextPath() %>/image/board/<%=atchVO.getStreFileNm() %>">
					</div>
					<%
						}
										}
	...
	<div id="buttonContainer">
			...
			<button type="submit" class="btn btn-outline-secondary" onclick="authCheck('<%= mv.getMem_id() %>', '<%= pv.getPostWriter() %>', <%=pv.getPostNo()%>); location.href='./update.do?postNo=<%=pv.getPostNo()%>'">수정</button>
			<button type="submit" class="btn btn-outline-secondary" onclick="authCheck('<%= mv.getMem_id() %>', '<%= pv.getPostWriter() %>', <%=pv.getPostNo()%>); delCheck(); location.href='./delete.do?postNo=<%=pv.getPostNo()%>'">삭제</button>
	</div>
...
<!-- 				<label for="commContent" id="commInsertCont">내용: </label> -->
	...
	<div id="commentsContainer" class="table-responsive">
	<table width=100% class="table table-striped">
				<%
					if(commList.size() == 0 ) {
				%>
				...
				<tr>
					...
					<td>
					<button type="submit" class="btn btn-outline-secondary btn-sm" onclick="authCheckComm('<%= mv.getMem_id() %>', '<%= pv2.getCommWriter() %>', <%=pv2.getCommNo()%>); location.href='./commUpdate.do?postNo=<%=pv2.getPostNo()%>&commNo=<%=pv2.getCommNo()%>'">수정</button>
					<button type="submit" class="btn btn-outline-secondary btn-sm" onclick="authCheckComm('<%= mv.getMem_id() %>', '<%= pv2.getCommWriter() %>', <%=pv2.getCommNo()%>); delCheck(); location.href='./commDelete.do?postNo=<%=pv2.getPostNo()%>&commNo=<%=pv2.getCommNo()%>'">삭제</button>
					</td>
				</tr>
					<%
					}
...
function authCheck(memId, postWriter, postNo) {
	if (postWriter != memId || postWriter == null || postWriter == "") {
...
function authCheckComm(memId, commWriter, commNo) {
	if (commWriter != memId || commWriter == null || commWriter == "") {
//  		alert("memId: " + memId + " | commWriter: " + commWriter)
		alert("권한이 없습니다.");
		e.preventDefault();
	} else{
// 		alert("memId: " + memId + " | commWriter: " + commWriter)
