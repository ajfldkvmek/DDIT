...
<%
	...
	MbVO mb = (MbVO)request.getAttribute("mb");
%>    
...
<!-- 	<table width=100% class="table table-striped"> -->
	<table width=100% class="table">
		...
		<tr>
			...
			<td id="detailCate" class="table-dark" colspan="5"><%=mb.getMb_title()%></td>
		</tr>
		<tr>
			<th id="detailCate" style="text-align:center; width:130px; background-color:whitesmoke">작성자</th>
			<td style="width:200px"><%=mb.getMem_id()%></td>
...
<%-- 			<td class="detailCate" width=15% colspan="2"><%=pv.getPostDate() %> --%>
			<td id="detailCate" style="text-align:center; width:130px"><%=mb.getMb_bdate()%>
			<th id="detailCate" style="text-align:center; width:130px; background-color:whitesmoke">조회수</th>
			<td id="detailCate" style="text-align:center; width:130px"><%=mb.getMb_cnt()%></td>
		</tr>
		<tr>
			<th id="detailCate" style="text-align:center; background-color:whitesmoke">내용</th>
			<td colspan="5"><%=mb.getMb_cont()%></td>
		</tr>
		<tr>
	...
	<div id="buttonContainer">
			<button type="button" class="btn btn-outline-secondary" onclick="location.href='./mbList?meetNo=<%=pageNo%>'">목록</button>
			<button type="submit" class="btn btn-outline-secondary" onclick="authCheck(); location.href='./mbUpdate?mb_no=<%=mb.getMb_no()%>'">수정</button>
			<button type="submit" class="btn btn-outline-secondary" onclick="authCheck(); delCheck(); location.href='./delete.do?postNo=<%=mb.getMb_no()%>'">삭제</button> 
	</div>
	...
	<%-- <div id="commnetInsertContainer" class="form-group">
		<form action="./commInsert.do" method="post" enctype="multipart/form-data" id="commentInsertForm">
			<input type="hidden" name="postNo" value="<%=mb.getPostNo()%>">
...
<!-- 				<label for="commWriter" id="commInsertWriter">작성자: </label> -->
				<input type="text" class="form-control" id="commWriter" name="commWriter"
					...
					title="댓글 작성자" value="<%=mem.getMem_id() %>" disabled>
...
<!-- 				<label for="commContent" id="commInsertCont">내용: </label> -->
 ...
 --%>
	<button type="button" class="btn btn-dark-soft" style="float:left;" onclick="location.href='<%=request.getContextPath()%>/meetList'">글목록</button>
