...
public class MyBatisTest {
	public static void main(String[] args) {
		...
		// 1. myBatis 환경설정파일을 읽어와  SqlSessionFactory 객체 생성하기
		...
		try {
			...
			if(cnt > 0) {
				System.out.println("update작업 성공 : " + cnt);
				sqlSession.commit();
			}else {
		...
		// 2-4. select 연습
		// 1) 응답 결과가 여러개일 경우...
		System.out.println("select 연습 시작(응답 결과가 여러개 일 경우)...");
		
		// 응답 결과가 여러개일 경우에는 selectList()를 이용한다.
		// 이 메서드는 여러개의 레코드를 VO에 담은 후 List에 추가해 주는 작업을
		// 대신 수행 해 준다.
		...
		try {
			List<MemberVO> memList = 
					sqlSession.selectList("memberTest.selectAll");
			
			for(MemberVO mv2 : memList) {
				System.out.println("I D : " + mv2.getMemId());
		...
		// 2) 응답이 1개인 경우
		System.out.println("select 연습 시작(응답 결과가 1개 일 경우)...");
		
		sqlSession = sessionFactory.openSession(true);
		
		// 응답결과가 1개일 경우에는 selectOne()메서드를 이용한다.
		mv = (MemberVO) sqlSession.selectOne("memberTest.getMember", "a001");
		...
		System.out.println("(응답 결과가 1개 일 경우) 출력 끝...");
