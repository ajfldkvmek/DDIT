...
public class SecurityController {
	 ...
	 * 		# 시큐리티 인증 구조
	 *
	 * 			클라이언트에서 타겟을 돌아가기 위해 요청을 진행한다. 이때, 타겟에 설정되어 있는 요청 접근
	 ...
	 * 			인증을 요청한다. 사용자는 아이디, 비밀번호를 입력 후 인증을 요청한다.
	 * 			클라이언트에서 서버로 요청한 HttpServletRequest의 요청 객체를 AuthenticationFilter
	 * 			요청을 intercept한다. UsernamePasswordAuthenticationToken을 통해 인증을 진행할
	 * 			토큰을 만들어 AuthenticationManager에게 위임한다.
	 * 			넘겨받은 id,pw를 이용해 인증을 진행하는데 성공 시, Authentication 객체 생성과 성공을 전달
	 * 			하고, 실패하면 Exception 에러를 발생시킨다. 인증에 성공 전 만들어진 Authentication
	 * 			객체를 AuthenticationProvider에게 전달하고 userDateilService에서 넘겨받은
	 ...
	 * 	
	 *  3. 접근권한 제한 설정
	 *  - 시큐리티 설정을 통해서 특정 URI에 대한 접근을 제한할 수 있따.
	 *  
	 *  	# 환경 설정	
	 *  		- 스프링 시큐리티 설정
	 *  		> URI패턴으로 접근 제한을 설정
	 ...
	 *  				pattern="/board/list" access="permitAll"/>
	 *  
	 *  		- <security:intercept-url 
	 *  				pattern="/board/register" access="hasRole('ROLR_MEMBER')"/>
	 *  
	 *  	
	 *  	# 화면 설명
	 *  		
	 *  		- 일반 게시판 목록화면(permitAll)				: 모두 접근
	 *  		- 일반 게시판 등록화면(hasRole('ROLR_MEMBER')) : 회원권한이상 접근
	 *  			> 접근제한이 걸려 스프링 시큐리티가 기본적으로 제공하는 로그인 페이지로 이동
	 *  		- 공지 게시판 목록화면(permitAll)				: 모두 접근
	 *  		- 공지 게시판 등록화면(hasRole('ROLR_ADMIN')) : 회원권한이상 접근
	 *  			> 접근제한이 걸려 스프링 시큐리티가 기본적으로 제공하는 로그인 페이지로 이동
	 *  
	 *  		
	 *  
	 ...
	 *  - 스프링 시큐리티 5버전 부터는 패스워드 암호화 처리기를 반드시 이용하도록 변경이 되었다
	 ...
	 *  			<security:authentication-privider>
	 ...
	 *  			> 관리자 등급인 admin 계정은 ROLE_MEMBER권한도 가지고 있는 계정이므로 해당 파이지로 접근 가능
	 *  
	 *  		- 공지사항 게시판 등록 화면
	 *  			> 접근 제한에 걸려 스프링 시큐리티가 기본적으로 제공하는 로그인 페이지가 연결되고,
	 *  			관리자 등급인 ROLE_AMIN권한을 가진 admin 계정으로 로그인 후 해당 페이지로 접근 가능
	 *  		
	 *  
	 *  
	 *   5. 접근 거부 처리
	 *   
	 *   - 접근 거부가 발생한 상황을 처리하는 거부처리자의 URI를 지정할 수 있따.ㄴ
	 *   
	 *   	# 환경 설정
	 ...
	 *   			> 접근 거부 처리자의 URI지정
	 *   			> <security:accesss-denied-handler error-page"/accessError"/>
	 *   
	 *   
	 *   
	 *   	# 접근 거부 처리
	 ...
	 *   
	 *   		- 공지사항 게시판 등록 페이지
	 *   			> 접근 제팡에 걸려 스프링 시큐ㅜ리티가 제공하는 로그인 페이지가 나타나고 ,
	 *   				회원 권한을 가진 계정으로 접근 시 공지사항 게시판 등록은 관리자만 가능하므로 접근이 거부됨
	 *   				이 때, access-denied-handler 로 설ㅈ어되어 있는 uri로 이동하고 
	 *   				해당 페이지에서 접근이 거부되었을때 보여질 페이지의 정보가 나타남
	 *   
	 *    
	 *    6. 사용자 정의 접근 처리 거부 처리자
	 *    - 접근 거부가 발생한 상화에 단순 메세지 처리 이상의 다양한 처리가 하고 싶다면 AccessDeniedHanlder를 직접 구현해야함
	 *    
	 *    		# 환경 설정
	 *    		- 스프링 시큐리티 설정(security-context.xml 설정)
	 *    			> id가 'customAccessDenied'를 가진 빈을 등록
	 *    			> <security:accss-denied-handler ref="customAccessDenied"/>
	 *    
	 *    
	 *    		# 접근 거부 처리자 클래스 정의
	 *    
	 ...
	 *    		- accessDeinedHandler 인터페이스를 참조받아서 handle 메소드를 재정의하여 사용
	 *    		우리는 접근이 거부되었을 때 빈으로 등록해둔 customAccessDeniedHandler 클래스가 발동해
	 *    		해당 메서드가 실행되고 response내장 객체를 활용하여 /accessError URL로 이동하여
	 *    		접근 거부시 보여질 페이지로 이동하지만 이곳에서 더 많은 로직을 처리하길 원한다면
	 *    		request, response 내장 객체를 이용하여 다양한 처리가 가능하다
	 *    
	 *    		# 화면 설명
	 *    		- 일반 게시판등록 페이지
	 *    		> 접근 제한에 걸려 스프링 시큐리티가 제공하는 로그인 페이지가 나타나고,
	 *    		회원 권한을 가진 계쩡으로 접근시 접근 가능
	 *    
	 *    		- 공지사항 게시판 등록 페이지
	 *			> 접근에 제한에 걸려 스프링 시큐리티가 제공하는 로그인 페이지가 나타나고, 회원 권한을 가진
	 *			계정으로 접근 시에 공지사항 게시판 등록 페이지는 관리자 권한만 접근 가능하므로 접근이 거부된다
	 *			이때, access-denied-handler로 셜정되어 있는 ref 속성에 부여된 클래스 메소드로
	 ...
	 *
	 *		# 환경 설정
	 *			- 스프링 시큐리티(security-context.xml 설정)
	 *			<security:form-login/> 삭제(기본 로그인 페이지로 사용하는게 아니기 때문에 주석)
	 *   		<security:form-login login-page="/login"/> 설정
	 *   		: 사용자가 직접만든 로그인 페이지로 이동할 URL을 가지고 있는 컨트롤러 메서드 정의
	 *   
	 *   	# 로그인 페이지 정의 
	 *   		- 사용자가 정의한 로그인 컨트롤러
	 *   			> controller 패키지안에 LoginController 메소드 생성
	 *   		- 사용자가 정의한 로그인 뷰
	 *   			> securit/loginForm.jsp
	 *   
	 *   
	 *   		** 시큐리티에서 제공하는 기본 로그인 페이지로 이동하지 않고, 사용자가 정의한 로그인 페이지의
	 *			URI를 요청하여 해당 페이지에서 권한을 체크하도록 합니다.
	 ...
	 *			authentication-success-hand1er-ref="customLoginSuccess"/> 추가
	 *
	 *
     *		# 로그인 성공 처리자 클래스 정의
	 *		- 로그인 성공 처리자
	 *		> SavedReuqestAwareAuthenticationSuccessHand1er는
	 *		AuthenticationSuccssHandler의 구현 클래스이다
	 *		인증 전체 접근을 시도한 URL로 리다이렉트 하는 기능을 가지고 있으며 
	 *		스프링 시큐리티에서 기본적으로 사용되는 구현 클래스이다
	 *
	 *		- 로그인 성공 처리자2
	 *		> AuthenticationSuccssHandler 인터페이스를 직접 구현하여 
	 *		인증 전에 접근을 시도한 URL로 리다이렉트 하는 기능을 구현
	 *
     *		# 화면 설명
     *		
     *		- 일반 게시판 등록 화면
	 ...
	 *
	 *	9. 로그아웃 처리
	 *	- 로그아웃 처리를 위한 URI를 지정하고 로그아웃 처리 후에 
	 *	별도의 작업을 하기 위해서 사용자가 직접 구현한 처리자를 등록할 수 있따.
	 *
	 *		# 환경 설정
	 *			- 스프링 시큐리티 설정(security-context.xml 설정)
	 *		true/false 설정가능
	 ...
	 *				기존 데이터베이스 연결을 위한 라이브러리가 주가되어 있다. (Spring2Project내)
	 *				
	 *			# 스프링 설정(root-context.xml 셜정)
	 *				- 데이터 소스 설정(기존 설정대로)
	 *				
	 *			# 스프링 시큐리티 실정(security-context.xml 실정)
	 *			
	 *				customPasswordEncoder 빈 등록 진행
	 *				> <security:authenticaton-manger> 태그 설정
	 *   
	 *   
	 *   
	 *			# 비밀번호 암호화 처리기 클래스 정의
	 ...
	 * 					제대로 할려면 생성된 사용자 테이블(users)에 비밀번호를 암호화하여 저장해야 한다.
	 * 					테스트를 위해서 생성한 데이터는 암호화를 처리하지 않고 로그인 하면 당연히 로그인 에러가
	 * 					발생할 것입니다. (암호화된 비밀번호가 날라가는게 아니라서)
	 * 					그래서 암호화를 하지 않는 PasswordEncoder를 직접 구현하여 지정하면 로그인 시
	 * 					암호화를 고려하지 않으므로 로그인이 정상적으로 이루어지는걸 확인할 수 있습니다.
	 *
	 ...
	 *   - 스프링 시큐리티가 기본적으로 이용하는 테이블 구조를 그대로 생성해도 사용해도 되지만
	 *   기존해 구축된 회원 테이블이 있다면 약간의 작업으로 기존 테이블을 활용할 수 있따.
	 *   
	 *   	# 테이블 준비
	 *   		
	 ...
	 *   
	 *   		** BCryptPasswordEncoder 클래스를 이용하여 직접 encode된 비밀번호를 찾아 db세팅
	 *   		
	 *   		- BCryptPasswordEncoder 클래스릃 ㅘㄹ용한 당방향 비밀번호 암호화
	 *   		> encode 메서드를 통해서 sha 방시의 8바이트 hash암호화를 매번 랜덤하게 생성
	 *   		> 같은 문자열을 반환하더도 매번 다른 문자열을 반환
	 *   
	 *   		비밀번호를 입력하면 암호화된 비밀번호로 인코딩되는데, 암호화된 비밀번호화 데이터베이스 내 존재하는
	 *   		테이블에 있는 암호화된 비밀번호가 일치한지를 파악후 일치하면 로그인 성공으로 간주하고 다음단계 진행
	 *   
	 *   		> BCryptPasswordEncoder 크래스의 encode()메서드를 통해 만들어지느 암호화된
	 *   		해쉬 다이제스트들은 입력한 비밀번호 문자에 해당하는 수십억개의 다에즈트중 일치하는 다이제스트가 존재하면
	 *   		인증성공
	 *   
	 *   
	 *   
	 ...
	 *   
	 *   	# 클래스 재정의
	 *   		- UserDetailsSercice 재정의
	 *   		> security.CustomUserDetailService
	 *   		> 기존 사용중인 read를 기반으로 한 readById 재정의
	 *   		> CustomUserDetailsService 클래스 내 loadUserByUsername 메소드에서
	 *   		인코딩된 비밀번호 확인 후 데이터 베이스 비밀번호 수정
	 *   			> Member 테이블 비밀번호 수정(11번에서 함)
	 *   
	 *   
	 *   
	 *   
	 ...
	 *   로그인한 사용자 정보를 보여줄 수 있다.
	 *   
	 *   	# 공동 표현식
	 *   		- hasRole([role])
	 *   			> 해당 롤이 있으면 true
	 *   		- hadAnyRole([role, role2])
	 *   			> 여러 롤들중에서 하나라도 해당하는 롤이 있으면 true
	 *   		- principal
	 *   			> 인증된 사용자의 사용자정보(UserDetails 인터페이스를 구현한 클래스의 객체)를 의미
	 *   
	 *   		- authentication	
	 *   			> 인증된 사용자의 이증정보(Authentication 인터페이스를 구현한 클래스의 객체)를 의미
	 *   
	 *   		- permitAll
	 *   			> 모든 사용자 허용
	 *   	
	 ...
	 *   	 
	 *   		- isFullAithenticated
	 *   			> remebmer-me로 인증된 것이 아닌 일반적인 방법으로 인증된 사용자의경우 true
	 *   
	 *   
	 *   
	 *   	# 표션식 사용
	 ...
	 *   
	 *   14. 자동 로그인
	 *   - 로그인 하면 특정 시간 동안 다시 로그인 할 필요가 없는 긴으
	 *   - 스프링 시큐리티느 메모리나 테이터베이스를 사용하여 처리
	 *   - 기능을 구현하기 위해 <security:remember-me> 태그를 이용하여 시큐리티 설정 파일을 수정
	 *   
	 *   	# 데이터베이스 테이블
	 *   		- persistemt_logins 테이블 준비
	 *   
	 *   	# 환경 설정
	 ...
	 *   		
	 *   		- JSESSIONID 와 rembmer-me 쿠키가 만들어진다
	 *   		- JSESSIONID를 삭제후 다시 로그인을 진행하더라도 
	 *   			로그인 후 진행될 페이지가 정상적으로 나타는 걸 확인할 수 있다.
	 *   			> 자동 로그인이 remember-me에 의해 실행됨을 확인할 수 있다.
	 *   
	 *   
	 *   
	 *   15. 스프링 시큐리티 어노테이션
	 *   - 스프링 시큐리티는 어노테이션을 사용하여필요한 설정을추가할 수 있다
	 *   	
	 *   	# 사용어노테이션
	 *   		
	 *   		- @Secured
	 ...
	 *   
	 *   		- @Preauthorize
	 *   			> 메서드 실행되기전에 적용할 접근 정책을 지정할 때 
	 *   
	 *   		- @postauthorize
	 *   			> 메서드 실행후 적용할 접근 정책을 지정할 때 
	 *   
	 *   
	 *   
	 *   
