...
<%-- 										id="title2" value="${board.title }" placeholder="제목을 입력해주세요."/> --%>
									</td>
								</tr>
								<tr>
									<td>
										<input class="form-control" type="text" name="writer"
										id="writer" value="${board.writer }" placeholder="작성자을 입력해주세요."/>
									</td>
								</tr>
								<tr>
									<td>
...
$(function(){
	...
	registerBtn.on("click", function(){
		...
		boardForm.submit();	// submit 이벤트로 날려 서버로 데이터를 전송한다.
	});
	
	// 취소 버튼
	cancelBtn.on("click", function(){
		location.href="/curd/board/read?boardNo=${board.boardNo}"
	});
