			...
			<form action="/notice/signup.do" method="post" id="signupForm"
				...
				<div class="input-group mb-3">
					<input type="text" class="form-control" id="memId" name="memId"
						value="${errors.memId }" placeholder="아이디를 입력해주세요"> <span
						class="input-group-append">
						<button type="button" class="btn btn-secondary btn-flat"
							id="idCheckBtn">중복확인</button>
					</span> <span class="error invalid-feedback" style="display: block;">${errors.memId }</span>
				</div>
				<div class="input-group mb-3">
					<input type="text" class="form-control" id="memPw" name="memPw"
						value="${errors.memPw }" placeholder="비밀번호를 입력해주세요"> <span
						class="error invalid-feedback" style="display: block;">${errors.memPw }</span>
				</div>
				<div class="input-group mb-3">
					<input type="text" class="form-control" id="memName" name="memName"
						value="${errors.memName }" placeholder="이름을 입력해주세요"> <span
						class="error invalid-feedback" style="display: block;">${errors.memName }</span>
				</div>
				<div class="input-group mb-3">
				...
				<div class="input-group mb-3">
					<input type="text" class="form-control" id="memEmail"
						name="memEmail" value="${errors.memEmail }"
						placeholder="이메일을 입력해주세요">
				</div>
				<div class="input-group mb-3">
					<input type="text" class="form-control" id="memPhone"
						name="memPhone" value="${errors.memPhone }"
						placeholder="전화번호를 입력해주세요">
				</div>
				<div class="input-group mb-3">
					<input type="text" class="form-control" id="memPostcode"
						value="${errors.memPostcode }" name="memPostcode"> <span
						class="input-group-append">
						<button type="button" class="btn btn-secondary btn-flat"
				...
				<div class="input-group mb-3">
					<input type="text" class="form-control" id="memAddress1"
						name="memAddress1" value="${errors.memAddress1 }"
						placeholder="주소를 입력해주세요">
				</div>
				<div class="input-group mb-3">
					<input type="text" class="form-control" id="memAddress2"
						name="memAddress2" value="${errors.memAddress2 }"
...
<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=b9bcddcb73f2341da01a2ab52c781b81&libraries=services"></script>
<script>
	$(function() {
		...
		var memAddress2 = $("#memAddress2"); // 상세 주소 엘리멘ㅌ느

		var idCheckFlag = false;//아이디 중복 체크했는지 확인하기위한 flag
		...
		idCheckBtn.on("click", function() {
			...
			$.ajax({
				...
				beforeSend: function(xhr){ //데이터 전송전 헤더에서 csrf값 확인
					//header과 token 은 loginTemplage.jsp 에서 
				...
				success : function(res) {
					...
					if (res == "NOTEXIST") {
						alert("사용가능 아이디");
						idCheckFlag = true; //중복확인 했다는 flag 설정값
					} else {
						alert("이미 사용중인 아이디");
					}
				}
		...
		//상세주소까지 입력된 후 포커스 전환됐을 때 지도를 띄운다
		memAddress2
				.focusout(function() {
					...
					if (address1 != null && address1 != "") {
						...
						// 주소로 좌표를 검색합니다
						geocoder.addressSearch(address1 + " " + address2,
		...
		signupBtn.on("click", function() {
			...
			// 아이디 비번 이름까지만 일반적인 데이터 검증
			// 개인정보 처리 방침 동의를 체크했을 때만 가입가능 하도록 
			// 중복확인 처리가 true일 때 만 가입하기 가능하도록 처리

			var agreeFlag = false; // 개인정보처리방침동의여부
			var memId = $("#memId").val();
			var memPw = $("#memPw").val();
	...
	function isImageFile(file) {
		...
		var ext = file.name.split(".").pop().toLowerCase(); //확장자 가져오기
