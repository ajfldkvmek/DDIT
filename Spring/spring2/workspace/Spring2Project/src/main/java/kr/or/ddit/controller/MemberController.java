...
//	private static final Logger log = LoggerFactory.getLogger(MemberController.class);
	...
	// 2) @RequestParam 어노테이션을 사용하요 특정한 HTML Form의 필드명을 지정하여 요청을 처리한다.
	...
	// 6) 복수 선택이 가능한 폼 셀렉트 박스 요소 값을 문자열 배열 타입 매개변수로 처리한다.
	 ...
	 * 			> pom.xml내 commons-fileupload, commom-io라이브러리 의존관계 추가
	 * 			> web.xml multipart 필터 추가
	 */
	// 1) 파일 업로드 폼 파일 요소값을 스프링 MVC가 지원하는 multipartFile매개변수로 처리
	...
	//2) 파일 업로드 폼 파일 요소값과 텍스트필드 요소값을 스프링MVC가 지원한 multipartFile 매개 변수와 문자열 매개변수로 처리
	...
	//3) 파일 업로드 폼 파일 요소값과 텍스트필드 요소값을 스프링MVC가 지원한 multipartFile 매개 변수와 자바빈즈로 처리
	...
	//4) 파일 업로드 폼 파일 요소값과 텍스트필드 FileMember 타입의 자바빈즈 매개변수로 처리하기
	...
	//5) 여러개의 파일을 받아서 multipartFile 매개변수로로 처리
	...
	public String registerFile05(MultipartFile picture1, MultipartFile picture2) {
		...
		log.info("picture1 name >> " + picture1.getOriginalFilename());
		log.info("picture1 fileSize>> " + picture1.getSize());
		log.info("picture1 contentType>> " + picture1.getContentType());
		
		log.info("picture2 originalName>> " + picture2.getOriginalFilename());
		log.info("picture2 fileSize>> " + picture2.getSize());
	...
	//6) 여러개의 파일업로드를 폼 파일 요소값을 multipartfile 타입의 요소를 가진 리스트로 처리
	...
	public String registerFile06(List<MultipartFile> pictureList) {
		...
		for(int i = 0; i < pictureList.size(); i++) {
			...
			log.info("picture name >> " + picture.getOriginalFilename());
			log.info("picture fileSize>> " + picture.getSize());
	...
	//7) 여러개의 파일업로드를 폼 파일 요소값을 MultiFileMember 타입의 자바빈즈 매개변수로 처리
	...
	public String registerFile07(MultiFileMember multiFileMember) {
		...
		//String userId = multiFileMember.getUserId();
		//String password = multiFileMember.getPassword();
		...
		log.info("multiFileMember.getPassword(): " + multiFileMember.getPassword());
	...
	//8) 파일업로드 폼 파일 요소값과 텍스트 필드 요소값을 multipartFile타입의 배열 매개변수로 처리한다
	...
	public String registerFile08(MultipartFile[] pictureArray) {
		...
		for(int i = 0; i < pictureArray.length; i++) {
			log.info("picture name >> " + pictureArray[i].getOriginalFilename());
			log.info("picture fileSize>> " + pictureArray[i].getSize());
			log.info("picture contentType>> " + pictureArray[i].getContentType());
		}
