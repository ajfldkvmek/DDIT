...
public class TilesSettiongsController {
	 ...
	 *  
	 *  
	 *  		** 페이지 모듈화를 Tiles를 함께 사용하여 CRUD 진행
	 *  
	 *  
	 *  
	 *  
	 *  	1. Tiles란?
	 *  
	 *  		- 어떤 jsp 템플릿으로 사용하고 템플릿의 각 영역을 어떤 내용으로 채울지에 대한 정보를 설정
	 *  		- 하나의 화면들을 만들다보면 공통적이로 반복적으로 생성해야 하는 header, footer와 같은 영역들이 존재
	 *  			우리는 그러한 공통분들을 분리하여 반복적으로 컴포넌트들을 사용하는게 아니라
	 *  			공통된 부분은 한 번만 가져다 쓰고 변화하는 부분에 대해서만 동적으로 변환해 페이지를 관리할 수 있어야함
	 *  			이렇게 header/footer/menu/side 등 공통적인 소스를 분리하여 한 화면에 동적으로 레이아웃을 한 곳에 배치하여
	 *  			설정하고 관리할 수 있또록 도와주는 페이지 모듈화를 돕는 프레임워크입니다.
	 *  
	 *  
	 * 			- 아래 jsp들을 이용하여 페이지 모듈화를 진행
	 * 			> template.jsp		: 대표적인 background 페이지(페이지 모듈화의 배경지가 될 페이지)
	 * 			> header.jsp		: 공통으로 중복되어 나올 헤더페이지
	 * 			> content source 	: content는 동적으로 변화할 페이지
	 * 			> footer.jsp		: 공통으로 중복되어 나올 footer페이지
	 * 
	 * 
	 * 
	 * 
	 * 		2. Tiles Layout 구현 설졍
	 * 
	 * 			1) Tiles 의존 관계 등록
	 * 				- tiles-core
	 * 				- tiles-api
	 * 				- tiles-servlet
	 * 				- tiles-jsp
	 * 			** 의존 관계 등록 후 Maven > Update Project
	 * 
	 * 			2) servlet-context.xml 수정
	 * 			- ViewResolver order 순서 2위로 지정
	 * 			- tilesViewResolver bean 등록 진행
	 * 
	 * 
	 * 			3) tiles 설정 위한 xml 생성
	 * 			- /WEB-INF/spring/tiles-config.xml 생성
	 * 
	 * 
	 * 			4) tiles xml에 설정한  layout 설정대로 페이지 생성
	 * 
	 * 
	 * 
	 ...
	 * 				- 게시판 등록 컨트롤러 만들기(notice/NoticeInsertContorller)
	 * 				- 게시판 등록 화면 컨트롤러 만들기(noticeForm:get)
	 * 				- 게시판 등록 화면 만들기(notice/form.jsp)
	 * 				
	 * 
	 * 				- 게시판 등록 기능 컨트롤러 만들기
	 * 				- 게시판 등록 기능 서비스 인터페이스 매서드 만들기
	 ...
	 * 				- 게시판 상세 화면 서비스 인터페이슨 메소드 만들기
	 * 				- 게시판 상세 화면 서비스 클래스 메소드 만들기
	 * 				- 게시판 상세 화면 Mapper 인터페이스 메소드 만들기
	 * 				- 게시판 상세 화면 Mapper xml 쿼리  만들기
	 * 				- 게시판 상세 화면 만들기(notice/deatil.jsp)
	 * 
	 * 
	 * 
	 * 				- 게시판 수정 화면 컨트롤러 만들기(noticeUpdateForm:get)
	 * 				- 게시판 수정 화면 만들기(notice/for.jsp)
	 * 				- 게시판 수정 화면 컨트롤러 메소드 만들기(noticeUpdate:post)
	 * 				- 게시판 수정 화면 서비스 인터페이슨 메소드 만들기
	 ...
	 * 				- 게시판 삭제 기능 서비스 인터페이슨 메소드 만들기
	 * 				- 게시판 삭제 기능 서비스 클래스 메소드 만들기
	 * 				- 게시판 삭제 기능 Mapper 인터페이스 메소드 만들기
	 * 				- 게시판 삭제 기능 Mapper xml 쿼리  만들기
	 * 
	 * 
	 * 				- 게시판 목록 서비스 인터페이슨 메소드 만들기
	 * 				- 게시판 목록 서비스 클래스 메소드 만들기
	 * 				- 게시판 목록 Mapper 인터페이스 메소드 만들기
	 * 				- 게시판 목록 Mapper xml 쿼리  만들기
	 * 
	 * 				
	 * 				
	 * 
